...
const EmployeeReadUserMessages = async (req, res) => {
    ...
    const query = "SELECT * FROM messages WHERE id = ? ORDER BY publication_date DESC";
...
const UserReadHisMessages = async (req, res) => {
    ...
    const query = "SELECT * FROM messages WHERE user_id = ? ORDER BY publication_date DESC";
    const [messages] = await Query.findByDatas(query, req.params);
    if(!messages.length){
        res.status(404).json({msg: "pas de message de cet utilisateur"})
    }
...
const ReadOneStatus = async (req, res) => {
    ...
    const query = "SELECT id , user_pseudo , user_email, content , status FROM messages WHERE id = ? ORDER BY publication_date DESC";
    const [messages] = await Query.findByDatas(query, req.params);
    if(!messages.length){
