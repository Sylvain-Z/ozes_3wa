...
const getOrdersByID = async (req, res) => {
    try {
    const query = "SELECT orders.id AS order_id, order_date, order_price, tracking_number, orders_products.priceEach, orders_products.quantity, products.id AS product_id, products.reference, products.title, MIN(pictures.id) AS first_picture_id, pictures.file_name, pictures.caption, pseudo, firstname, lastname, email, number, street, complement, postal_code, city, phone FROM orders JOIN orders_products ON orders_products.order_id = orders.id JOIN products ON products.id = orders_products.product_id JOIN pictures ON products.id = pictures.product_id JOIN users ON orders.user_id = users.id WHERE pictures.id = ( SELECT MIN(pictures.id ) FROM pictures WHERE pictures.product_id = products.id) AND orders.id = ? GROUP BY products.id, order_date, order_price, tracking_number, orders_products.priceEach, orders_products.quantity, products.reference, products.title, pictures.file_name, pictures.caption, pseudo, firstname, lastname, email, number, street, complement, postal_code, city, phone ORDER BY products.id ASC";
    const [datas] = await Query.findByDatas(query, req.params);
    if(!datas.length){
...
const getOrdersByUserID = async (req, res) => { 
    try {
    const query = "SELECT orders.id AS order_id, order_date, order_price, tracking_number, orders_products.priceEach, orders_products.quantity, products.id, products.reference, products.title, MIN(pictures.id) AS first_picture_id, pictures.file_name, pictures.caption, users.id AS user_id, firstname, lastname, email, number, street, complement, postal_code, city, phone FROM orders JOIN orders_products ON orders_products.order_id = orders.id JOIN products ON products.id = orders_products.product_id JOIN pictures ON products.id = pictures.product_id JOIN users ON users.id = orders.user_id WHERE pictures.id = ( SELECT MIN(pictures.id ) FROM pictures WHERE pictures.product_id = products.id) AND user_id = ? AND order_id = ? GROUP BY products.id, order_date, order_price, tracking_number, orders_products.priceEach, orders_products.quantity, products.reference, products.title, pictures.file_name, pictures.caption, pseudo, firstname, lastname, email, number, street, complement, postal_code, city, phone ORDER BY products.id ASC";
    const [datas] = await Query.findByDatas(query, req.params);
    if(!datas.length){
        res.status(404).json({msg: "produits non reconnu"})
    }
